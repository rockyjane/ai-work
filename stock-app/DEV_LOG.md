...
> 「股票分析網站」專案**專屬**的詳細開發紀錄。面試時「遇到問題→怎麼 debug→怎麼解」的故事，比功能本身更能展現實力。
>
> - 跨專案**簡要摘要**在上層 [`../DEV_LOG.md`](../DEV_LOG.md)；環境／工具搭建的雷在 [`../CLAUDE_CODE_SETUP_GUIDE.md`](../CLAUDE_CODE_SETUP_GUIDE.md)。
> - **結構固定為四大要點**（一、完成 ／ 二、坑&解法 ／ 三、實測 ／ 四、現況）。每到一個進度點，就在 一～三 各加上有時間戳的條目 `### 日期 時間 [寫入者] 分類`（分類：完成／雷／決策／觀念／實測），並更新「四、現況與下一步」。
> - 「坑&解法」每筆**至少**要有 **現象→原因→解法**，鼓勵再補上好處、可用組合等額外資訊。
