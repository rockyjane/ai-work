...
- **狀態**：後端 MVP 完成並驗證；**搜尋涵蓋全台股（3088 檔）、台灣50 有 K 線**；**已實跑於 Docker MySQL 8.4（`.env` 一行切換 SQLite⇄MySQL）**；前端程式碼齊全，待啟動。
...
## ai-content-studio — AI 內容工作室（副業／被動收入）

- **狀態**：發想／籌劃階段。第一部作品《夢境編號：0》正在鎖定故事設定。
- **定位**：多 AI 角色分工的內容產線，先小說／旁白影片（YouTube）、後漫畫；差異化＝解決 AI 內容「角色不定型／劇情有洞／質感不一」。
- **關鍵決策**：首發選旁白影片（成本低先跑通產線）、時代設定架空現代（避法律風險）、架構按「產線階段」拆而非一作一 agent（知識庫才能累積）。
- **詳細紀錄** → [`ai-content-studio/DEV_LOG.md`](ai-content-studio/DEV_LOG.md)
